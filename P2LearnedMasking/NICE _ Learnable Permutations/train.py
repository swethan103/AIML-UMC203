...
def get_digits_dataloaders(batch_size, test_size=0.2, random_state=42):
    digits = load_digits()
...
def evaluate(flow, dataloader, device):
    flow.eval()
...
def main(args):
    ...
    for epoch in range(1, args.epochs + 1):
        train_metrics = train_one_epoch(
            ...
            checkpoint_every_minibatches=args.checkpoint_every_minibatches,
            epoch=epoch,
            global_step=global_step,
    ...
    plt.xlabel("Epoch")
    plt.ylabel("NLL")
    plt.title("Digits NICE with Per-Layer Bandit Permutations")
    plt.legend()
    plt.grid(True)
    plt.tight_layout()
...
if __name__ == "__main__":
    ...
    parser.add_argument("--checkpoint_every_minibatches", type=int, default=10)
    ...
    parser.add_argument("--data_bins", type=float, default=1.0)
