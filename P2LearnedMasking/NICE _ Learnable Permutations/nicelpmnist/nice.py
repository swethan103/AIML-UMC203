...
def _coord_to_index(coord, image_size):
    r, c = coord
    return r * image_size + c


def _apply_transform(coords, image_size, transform_name):
    last = image_size - 1
...
def _raster_order(image_size):
    return [(r, c) for r in range(image_size) for c in range(image_size)]


def _snake_rows_order(image_size):
    coords = []
    for r in range(image_size):
        cols = range(image_size) if r % 2 == 0 else range(image_size - 1, -1, -1)
        for c in cols:
            coords.append((r, c))
    return coords


def _snake_cols_order(image_size):
    coords = []
    for c in range(image_size):
        rows = range(image_size) if c % 2 == 0 else range(image_size - 1, -1, -1)
        for r in rows:
            coords.append((r, c))
    return coords


def _diag_sum_order(image_size):
    coords = [(r, c) for r in range(image_size) for c in range(image_size)]
    coords.sort(key=lambda x: (x[0] + x[1], x[0], x[1]))
    return coords


def _spiral_order(image_size):
    coords = []
    top, bottom = 0, image_size - 1
    left, right = 0, image_size - 1
...
def _quadrant_order(image_size):
    coords = []

    mid_low = image_size // 2
    mid_high = image_size - mid_low

    row_blocks = [
        range(0, mid_low),
        range(0, mid_low),
        range(mid_low, image_size),
        range(mid_low, image_size),
    ]
    col_blocks = [
        range(0, mid_low),
...
def build_permutation_bank(image_size, num_permutations=40):
    base_orders = [
        ("raster", _raster_order(image_size)),
        ("snake_rows", _snake_rows_order(image_size)),
        ("snake_cols", _snake_cols_order(image_size)),
        ("diag_sum", _diag_sum_order(image_size)),
        ("spiral", _spiral_order(image_size)),
        ("quadrant", _quadrant_order(image_size)),
    ]
    ...
    for base_name, base in base_orders:
        for transform_name in transforms:
            transformed = _apply_transform(base, image_size, transform_name)
            perm = tuple(_coord_to_index(coord, image_size) for coord in transformed)

            if len(set(perm)) != expected_dim:
...
class NICE(nn.Module):
    def __init__(
        ...
        expected_dim = image_size * image_size
        if in_out_dim != expected_dim:
            raise ValueError(
                f"in_out_dim must equal image_size * image_size = {expected_dim}, "
                f"but got {in_out_dim}."
            )

        perm_bank, perm_names = build_permutation_bank(
