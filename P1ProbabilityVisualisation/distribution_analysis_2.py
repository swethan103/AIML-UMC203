...
from utils import get_dataloader, get_dataloader_test
...
def plot_layer_vs_distance(distances_dict, metric,title="Layer vs Distance to Gaussian"):
    """
    ...
    plt.grid()
    plt.tight_layout()
    plt.savefig(f"layer_vs_distance{metric}.png")
    plt.show()
...
def plot_pca_evolution(model, data_loader, device, model_type="nice", layers_to_plot=None):
    """
    Fixed-basis PCA visualization:
    - Fit PCA ONCE on final latent layer + Gaussian reference
    - Project all intermediate layers into same PCA basis

    This makes layer-to-layer PCA plots directly comparable.
    """
    model.eval()
    ...
    # Flatten all intermediates
    zs = [z.reshape(z.size(0), -1).cpu().numpy() for z in zs]

    num_layers = len(zs)

    if layers_to_plot is None:
        layers_to_plot = sorted(set([
            0,
            num_layers // 4,
            num_layers // 2,
            3 * num_layers // 4,
    ...
    # --------------------------------------------------
    # FIT PCA ON FINAL LATENT + GAUSSIAN REFERENCE
    # --------------------------------------------------
    z_final = zs[-1]
    z_ref = np.random.randn(*z_final.shape)

    pca_fit_data = np.concatenate([z_final, z_ref], axis=0)

    pca = PCA(n_components=2)
    pca.fit(pca_fit_data)

    # Project Gaussian reference once
    ref_pca = pca.transform(z_ref)

    # --------------------------------------------------
    # PROJECT ALL LAYERS USING SAME PCA BASIS
    # --------------------------------------------------
    for l in layers_to_plot:

        if l >= num_layers:
            print(f"Layer {l} not available (model has {num_layers} layers). Skipping.")
            continue

        z_pca = pca.transform(zs[l])
        ...
        plt.title(f"Fixed-Basis PCA at Layer {l}")
        plt.xlabel("PC 1")
        plt.ylabel("PC 2")
        plt.legend()
        plt.grid()
        plt.tight_layout()
        plt.savefig(f"fixed_pca_layer_{l}.png")
        plt.show()
# =========================================================
# 7. MAIN
# =========================================================
...
if __name__ == "__main__":
    ...
    DATASET    = "mnist"
    BATCH_SIZE = 512
    MODEL_TYPE = "nice"         # "nice" | "realnvp" | "glow"

    CHECKPOINT_PATH = f"checkpoints_Gaussian_unconstrained/best_{DATASET}.pt"

    METRICS = ["covariance"]      # "wasserstein" | "mmd"
    ...
    for metric in METRICS:
        ...
        plot_layer_vs_distance({"NICE": distances}, metric, title = f"{metric}: Layer vs Distance to Gaussian")
    ...
    plot_pca_evolution(
        ...
        device=DEVICE,
        model_type=MODEL_TYPE,
        layers_to_plot=[0, len(layers)//2, final_layer],
    )
