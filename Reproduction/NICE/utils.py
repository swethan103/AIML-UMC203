...
def get_transform():
    """
    Shared transform for all standard datasets.
...
def get_quickdraw_dataset(category='cat', root='./data', max_items=70000):
    """
    Downloads and prepares QuickDraw numpy bitmaps.
    """
    os.makedirs(root, exist_ok=True)
    filepath = os.path.join(root, f'{category}.npy')
    
    if not os.path.exists(filepath):
        print(f"Downloading QuickDraw '{category}' dataset...")
    ...
    # Scale to [0, 1] to match transforms.ToTensor() behavior
    data = data.astype(np.float32) / 255.0
    
    # QuickDraw data is already flattened (N, 784), so we just wrap it
    data_tensor = torch.from_numpy(data)
    
    # Create dummy labels for compatibility with (x, _) loop in train.py
    labels_tensor = torch.zeros(len(data), dtype=torch.long)
...
def get_dataloader(dataset_name, batch_size=200):
    ...
    if dataset_name == 'mnist':
        ...
        ds = get_quickdraw_dataset(category='bowtie', root='./data', max_items=20000)
        train_ds = Subset(ds, range(0, 16000))
...
def get_dataloader_valid(dataset_name, batch_size=200):
    ...
    if dataset_name == 'mnist':
        ...
        ds = get_quickdraw_dataset(category='bowtie', root='./data', max_items=20000)
        valid_ds = Subset(ds, range(18000,20000))
...
def get_dataloader_test(dataset_name, batch_size=200):
    ...
    if dataset_name == 'mnist':
        ...
        ds = get_quickdraw_dataset(category='bowtie', root='./data', max_items=20000)
        # Using 60k-70k as the test split
        ds = Subset(ds, range(18000, 20000))
