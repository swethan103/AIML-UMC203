...
def test(epoch, net, testloader, device, loss_fn, num_samples):
    ...
    # --- MOVE THIS HERE (Outside all IF blocks) ---
    # This ensures 'state' always exists before you try to save it.
    state = {
        'net': net.state_dict(),
    ...
    if epoch % 30 == 0:
        os.makedirs('ckpts', exist_ok=True)
        # This will now work because 'state' was defined above!
        torch.save(state, f'ckpts/checkpoint_epoch_{epoch}.pth.tar')
